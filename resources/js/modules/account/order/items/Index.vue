...
const queryData = (params) => {
  return window.axios.get('/account/orders/items', {
    params: {
        search: search.value,
        ...params,
    },
...
const { data, current, totalPage, loading, pageSize, run, refresh } =
  usePagination(queryData, {
    formatResult: (res) => {
      // res.data is { data: PaginatorObj, success: true }
      // PaginatorObj has .data (array) and .total
...
const formatCurrency = (value) => {
   // Use window.$formatCurrency if available, or a simple fallback
...
const viewDetail = (record) => {
    window.location.href = `/account/orders/items/${record.code}`
}
</script>
  ...
  <div class="item-order-index">
    <div class="card">
      <header class="card-header noborder">
        <h4 class="card-title">Danh sách vật phẩm</h4>
      </header>
      <div class="card-body px-6 pb-6">
        <div class="mb-5 flex flex-col gap-5 md:flex-row md:items-center">
          <div class="ltr:ml-auto rtl:mr-auto flex gap-x-2">
            <a-input v-model:value="search" placeholder="Tìm theo tên hoặc mã" />
            ...
            <template #bodyCell="{ column, text, record }">
                <template v-if="column.dataIndex === 'name'">
                    <span class="font-medium text-slate-600 dark:text-slate-300" :title="text">
                        {{ text.replace('(ROBUX 120H)', '').trim().length > 50 ? text.replace('(ROBUX 120H)', '').trim().substring(0, 50) + '...' : text.replace('(ROBUX 120H)', '').trim() }}
                    </span>
               </template>
               ...
               <template v-if="column.dataIndex === 'created_at'">
                ...
                <template v-if="column.dataIndex === 'status'">
                    ...
                    <a-tag color="orange" v-else-if="['pending', '0', 'assigned'].includes(String(record.status).toLowerCase())">Chờ xử lý</a-tag>
