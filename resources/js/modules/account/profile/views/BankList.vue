<script setup>
import { computed, ref } from 'vue'
import { usePagination } from 'vue-request'
import axios from 'axios'
import moment from 'moment'
import { ReloadOutlined } from '@ant-design/icons-vue'

const props = defineProps({
  filters: {
    type: Object,
    default: () => ({}),
  },
})

const search = ref('')
const columns = computed(() => {
  const t = (key) => typeof window.$__t === 'function' ? window.$__t(key) : key;
  return [
    {
      title: t('Số Dư Trước'),
      dataIndex: 'balance_before',
    },
    {
      title: t('Số Tiền'),
      dataIndex: 'amount',
    },
    {
      title: t('Số Dư Sau'),
      dataIndex: 'balance_after',
    },
    {
      title: t('Nội Dung'),
      dataIndex: 'content',
      width: '40%'
    },
    {
      title: t('Mã Giao Dịch'),
      dataIndex: 'code',
      width: '15%'
    },
    {
      title: t('Thời Gian'),
      dataIndex: 'created_at',
      sorter: true,
    },
  ]
})

const queryData = (params) => {
  return axios.get('/api/accounts/transactions', {
    params: {
      search: search.value,
      ...props.filters,
      ...params,
    },
  })
}

const { data, current, totalPage, loading, pageSize, run, refresh } =
  usePagination(queryData, {
    formatResult: (res) => {
      const { data, meta } = res.data.data

      return {
        data: data ?? [],
        totalPage: meta.total_rows ?? 0,
      }
    },
    defaultParams: [
      {
        sort_by: 'id',
        sort_type: 'desc',
        limit: 10,
      },
    ],
    pagination: {
      currentKey: 'page',
      pageSizeKey: 'limit',
    },
  })

const formatDate = (date, format = null) => {
  if (date === null) {
    return ''
  }
  const parsedDate = moment(date, moment.ISO_8601)
  if (!parsedDate.isValid()) {
    return ''
  }
  if (format) {
    return parsedDate.format(format)
  }
  return parsedDate.format('HH:mm:ss - DD/MM/YYYY')
}

const formatCurrency = (number, currency = 'VND', maxinum = 2) => {
  return $formatCurrency(number, currency, maxinum)
}

const formatContent = (content) => {
    // 1. "Nạp thẻ thành công #SERI; phí 9%" -> "Nạp thẻ thành công #SERI - phí 9%" (Hide if 0%)
    if (content.includes('Nạp thẻ thành công')) {
        let newContent = content.replace('; phí', ' - phí');
        // Hide fee if 0%
        if (newContent.includes('0%')) {
             newContent = newContent.replace(/ - phí 0%/, '');
             newContent = newContent.replace(/- phí 0%/, '');
        }
        return newContent;
    }

    // 2. "AUTO Deposit MBBANK - TRANSID - Rev: 1000 - Discount: 10%" -> "MBBANK - TRANSID - Discount: 10%"
    if (content.includes('AUTO Deposit')) {
        // Remove prefix
        let newContent = content.replace('AUTO Deposit ', '');
        
        // Remove " - Rev: xxx"
        newContent = newContent.replace(/ - Rev: [\d.,]+( [A-Z]+| ₫)?/, '');
        newContent = newContent.replace(/ - Rev: [\d.,]+/, '');

        // Hide Discount if 0%
        if (newContent.includes('Discount: 0%')) {
            newContent = newContent.replace(' - Discount: 0%', '');
        }
        return newContent;
    }

    return content;
}

const dataSource = computed(() => data.value?.data ?? [])

const pagination = computed(() => ({
  current: current.value,
  total: totalPage.value,
  pageSize: pageSize.value,
}))

const handleTableChange = (pag, filters, sorter) => {
  run({
    page: pag?.current,
    limit: pag.pageSize,
    sort_by: sorter.field,
    sort_type: sorter.order === 'ascend' ? 'asc' : 'desc',
    ...filters,
  })
}
</script>

<template>
  <div class="overflow-auto">
    <div class="mb-5 flex flex-col gap-5 md:flex-row md:items-center">
      <div class="ltr:ml-auto rtl:mr-auto flex gap-x-2">
        <a-input v-model:value="search" placeholder="Search..." />
        <a-button type="primary" :loading="loading" @click="refresh">
          <i class="fas fa-search"></i>
        </a-button>
      </div>
    </div>
    <a-table :dataSource="dataSource" :columns="columns" :loading="loading" :pagination="pagination" size="small" @change="handleTableChange" class="font-medium whitespace-nowrap">
      <template #bodyCell="{ column, text, record }">
        <template v-if="column.dataIndex === 'code'">
          <b style="color: #191D88" class="copy cursor-pointer" :data-clipboard-text="text">{{ text }}</b>
        </template>
        <template v-if="column.dataIndex === 'balance_before'">{{
          formatCurrency(text)
        }}</template>
        <template v-if="column.dataIndex === 'amount'">
          <span :class="record.prefix === '-' ? 'text-danger-600' : 'text-green-600'">{{ record.prefix }} {{ formatCurrency(text) }}</span>
        </template>
        <template v-if="column.dataIndex === 'balance_after'">{{
          formatCurrency(text)
        }}</template>
        <template v-if="column.dataIndex === 'content'">
            {{ formatContent(text) }}
        </template>
        <template v-if="column.dataIndex === 'created_at'">{{
          formatDate(text)
        }}</template>
      </template>
    </a-table>
  </div>
</template>
