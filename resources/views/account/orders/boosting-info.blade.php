    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="bg-white dark:bg-[#1e1e2d] border border-gray-200 dark:border-[#2d2d3a] rounded-lg p-5 h-full shadow-sm">
            ...
            @php
                ...
                if ($warrantyExpireAt) {
                    $warrantyHours = $warrantyExpireAt->diffInHours($item->created_at);
                } else {
                    $warrantyHours = $item->package->warranty_hours ?? 0;
                }
            ...
            <div class="space-y-3">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="flex items-start gap-2">
                        ...
                        <div class="text-base font-medium text-gray-900 dark:text-white uppercase">{{ $item->name ?? '-' }}</div>
                    </div>
                     <div class="flex items-center gap-2">
                        <div class="text-base font-bold text-gray-500 dark:text-gray-300 min-w-[80px]">{{ __t('Loại:') }}</div>
                        <div class="text-base font-medium text-gray-900 dark:text-white">{{ __t('Cày thuê') }}</div>
                    </div>
                </div>
            ...
            @if($isWarrantyActive && !$hasWarrantyTicket)
                <div class="mt-3" id="warranty_section">
                    <div id="app-warranty-request" 
                        data-code="{{ $item->code }}" 
                        data-category="Bảo hành cày thuê" 
        ...
        <div class="bg-white dark:bg-[#1e1e2d] border border-gray-200 dark:border-[#2d2d3a] rounded-lg p-5 h-full shadow-sm">
            ...
            <div class="space-y-1">
                ...
                <!-- Cookie / 2FA / Liên Hệ -->
                @php
                    $extraLabel = 'Cookie / 2FA';
                    if (($item->group->slug ?? '') === 'cay-thue-blox-fruits') {
                        $extraLabel = 'Liên hệ';
                    }
                @endphp
                <div>
                    <span class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1.5 block">{{ __t($extraLabel) }}</span>
                    <div class="flex items-center h-[32px]">
                        <div class="relative bg-gray-50 dark:bg-[#2b2b40] border border-gray-200 dark:border-[#3f3f50] rounded-md flex-1 flex items-center overflow-hidden h-full">
                            <input type="text" value="{{ $item->input_extra }}" readonly class="bg-transparent {{ $extraLabel === 'Liên hệ' ? 'text-gray-500 dark:text-gray-400' : 'text-gray-900 dark:text-white' }} font-bold w-full py-1.5 px-2 text-[12px]" style="border: none !important; outline: none !important; box-shadow: none !important;" />
                            @if($extraLabel !== 'Liên hệ')
                            <button class="text-gray-400 hover:text-gray-900 dark:hover:text-white px-3 transition-colors focus:outline-none flex items-center justify-center h-full border-l border-gray-200 dark:border-[#3f3f50]" onclick="copyToClipboard('{{ $item->input_extra }}', '{{ $extraLabel }}')">
                                <iconify-icon icon="heroicons-outline:clipboard-copy" class="text-lg"></iconify-icon>
                            </button>
                            @endif
                        </div>
                ...
                <!-- Contact Info -->
                @if (($item->group->slug ?? '') !== 'cay-thue-blox-fruits' && !empty($item->input_contact) && $item->input_contact !== '-')
                <div>
  ...
  @push('scripts')
    ...
    <script>
      ...
      async function saveNote() {
          ...
          try {
              const response = await fetch('{{ route('account.orders.boosting.update-note') }}', {
                  method: 'POST',
                  headers: {
      ...
      function openWarrantyTicket() {
          const type = '{{ $item->package->name ?? "Cày thuê" }}';
