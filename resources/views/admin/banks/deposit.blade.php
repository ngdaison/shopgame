...
@section('content')
  <div class="row mb-3">
    <div class="col-12 d-flex justify-content-between align-items-center">
        ...
        <div>
            <a href="{{ route('admin.deposit.banks.config') }}" class="btn btn-purple btn-sm shadow-sm">
  ...
  <div class="row">
    ...
    <div class="col-md-7 mb-3">
        <div class="card h-100 shadow-sm mb-0">
            <div class="card-body">
                <div class="chart-title mb-3">THỐNG KÊ NẠP TIỀN THÁNG {{ now()->format('m') }}</div>
  ...
  <div class="card custom-card shadow-sm">
      <div class="card-header border-bottom-0 pb-0 d-flex justify-content-between align-items-center">
        <div class="section-title">LỊCH SỬ NẠP TIỀN TỰ ĐỘNG</div>
      ...
      <div class="card-body">
        <div class="mb-2">
            <form id="filter" onsubmit="$('#basic-1').DataTable().ajax.reload(); return false;">
              <input type="hidden" id="type" value="deposit-bank">
              <div class="mb-3 row">
                <div class="col-md-2">
                ...
                <div class="col-md-2">
                    ...
                    <select class="form-control" id="bank_code" name="bank_code">
                        ...
                        @foreach($banks as $bank)
                            <option value="{{ $bank->bank_code ?? $bank->number }}">{{ $bank->bank_name ?? $bank->name ?? $bank->number }} - {{ $bank->number }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-2">
                ...
                <!-- Additional filters specific to banks could go here if supported by Transaction API, 
                     but we stick to standard Transaction keys for now -->
...
<script>
    ...
    $(document).ready(function() {
      ...
      $("#basic-1").DataTable({
        ...
        serverSide: true,
        ajax: {
          url: '{{ route('admin.deposit.bank.api') }}',
          type: 'GET',
          headers: {
          ...
          data: function(d) {
             ...
             // Custom filters
             // d.type = 'banks'; // Not needed for specific controller 
             // d.code_prefix = ... removed
             d.username = $('#username').val();
             d.trans_id = $('#code').val(); // Map input 'code' to API 'trans_id' or 'code'
             d.bank_code = $('#bank_code').val();
             d.domain = $('#domain').val();
        ...
        columns: [{
          ...
          render: function(data, type, row) {
            if (row.domain_display) {
              return `<span class="badge badge-info">${row.domain_display}</span>`;
            } else {
          ...
          render: function(data, type, row) {
            @if(auth()->user()->hasRole('Product Manager'))
                return '';
            @endif
            return `<button class="btn btn-sm btn-danger" onclick="deleteHistory(${data})"><i class="fa fa-trash"></i></button>`;
          }
        }],
        columnDefs: [{
          orderable: false,
    ...
    function deleteHistory(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.post('{{ route('admin.deposit.banks.delete.api') }}', {
                    ids: [id]
                }, {
                    headers: {
                        'Authorization': (typeof userData !== 'undefined' && userData && userData.access_token) ? 'Bearer ' + userData.access_token : ''
    ...
    function clearAllHistory() {
        Swal.fire({
            ...
            text: "Toàn bộ lịch sử nạp tiền (trừ Product Manager) sẽ bị ẩn khỏi Admin!",
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                axios.post('{{ route('admin.deposit.banks.clear-all') }}', {}, {
                    headers: {
                        'Authorization': (typeof userData !== 'undefined' && userData && userData.access_token) ? 'Bearer ' + userData.access_token : ''
                    }
                }).then(res => {
