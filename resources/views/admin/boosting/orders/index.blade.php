          ...
          <tbody>
            @foreach ($orders as $item)
              <tr>
                ...
                <td>{{ $item->username }}</td>
                <td>
                  <span class="{{ in_array($item->status, ['Cancelled', 'Destroyed']) ? 'text-danger' : 'text-success' }}">
                    {{ Helper::formatCurrency($item->payment) }}
                  </span>
                </td>
      ...
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form action="{{ route('admin.boosting.orders.update', ['id' => $item->id]) }}" method="POST" enctype="multipart/form-data" class="default-form axios-form" data-reload="true">
              @csrf
              <div class="mb-3">
              ...
              @endif
              <div class="mb-3">
                <label for="input_extra" class="form-label">Ghi chú [Cookie / 2FA]</label>
                <textarea class="form-control" id="input_extra" name="input_extra" rows="1" readonly>{{ $item->input_extra }}</textarea>
              </div>
              <div class="mb-3">
  ...
  <script>
    ...
    const deleteOrder = async (id) => {
      ...
      try {
        const {
          data: result
        } = await axios.post('{{ route('admin.boosting.orders.delete') }}', {
          id
        })
    ...
    const clearAllOrders = async () => {
        ...
        try {
            const { data: result } = await axios.post('{{ route('admin.boosting.orders.clear-all') }}');
