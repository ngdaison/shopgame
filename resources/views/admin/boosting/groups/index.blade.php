...
@section('content')
  <div class="card custom-card">
    <div class="card-header justify-content-between">
      <div class="card-title">Quản lý nhóm của Dịch vụ cày thuê</div>
      <div class="d-flex flex-wrap gap-2">
        <form action="{{ route('admin.boosting.groups') }}" method="GET" class="d-flex gap-2">
            ...
            @foreach ($groups as $item)
              <tr>
                ...
                <td>
                  ...
                  <a href="{{ route('admin.boosting.packages', ['id' => $item->id]) }}" class="badge bg-primary-gradient text-white me-1"><i class="fa fa-eye"></i> Xem</a>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form action="{{ route('admin.boosting.groups.update') }}" method="POST" enctype="multipart/form-data" class="default-form axios-form" data-reload="true">
              @csrf
  ...
  <div class="modal fade" id="modal-create" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="{{ route('admin.boosting.groups.store') }}" method="POST" enctype="multipart/form-data" class="default-form axios-form" data-reload="true">
            @csrf
            <div class="mb-3">
              <label for="category" class="form-label">Chuyên mục</label>
  ...
  <script>
    const deleteRow = async (id) => {
      ...
      try {
        const {
          data: result
        } = await axios.post('{{ route('admin.boosting.groups.delete') }}', {
          id,
          _token: '{{ csrf_token() }}'
        })
    ...
    const updatePriority = $debounce(function(id, val) {
        axios.post('{{ route('admin.boosting.groups.update-priority') }}', {
            id: id,
            priority: val,
            _token: '{{ csrf_token() }}'
        }).then(res => {
