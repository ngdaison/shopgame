...
@section('content')
  <div class="card custom-card">
    <div class="card-header justify-content-between">
      <div class="card-title">Cập nhật sản phẩm "{{ $item->code }}"</div>
    </div>
    <div class="card-body">
      <form action="{{ route('staff.products.accounts.items.update', ['id' => $item->id]) }}" method="POST" enctype="multipart/form-data" class="axios-form" data-reload="true">
        @csrf
        <div class="mb-3">
          ...
          <div class="row">
            @foreach ($item->list_image as $key => $img)
              <div class="col-md-3 mb-4 text-center" data-file-id="img-{{ $key }}">
                <img src="{{ Helper::getValidImage($img) }}" style="width: 100%; height: 100%;">
        ...
        <div class="mb-3 text-center">
          ...
          <br />
          <br />
          <a href="{{ route('staff.products.accounts.items', ['id' => $item->group_id]) }}" class="btn btn-danger"><i class="fa fa-arrow-left"></i> Quay lại ngay</a>
        </div>
      </form>
    </div>
  </div>
