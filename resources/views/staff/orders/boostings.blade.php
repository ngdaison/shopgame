...
@section('content')
  <div class="card custom-card">
    <div class="card-header justify-content-between">
      <div class="card-title">Đơn hàng Cày thuê - Đã nhận</div>
    </div>
    <div class="card-body">
                  ...
                  <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        ...
                        <form action="{{ route('staff.orders.boostings.update', ['id' => $item->id]) }}" method="POST" enctype="multipart/form-data" class="axios-form" data-reload="true">
                          @csrf
                          <div class="mb-3">
                          ...
                          @if (in_array($item->status, ['Processing', 'Completed', 'Cancelled']))
                          ...
                          @else
                            <input type="hidden" name="admin_note" value="{{ $item->admin_note }}">
                            <input type="hidden" name="order_note" value="{{ $item->order_note }}">
                          @endif
                          <div class="mb-3">
                            <label for="status" class="form-label">Trạng thái</label>
                            <select class="form-select" id="status" name="status" required>
                              @if ($item->status === 'Assigned')
                                <option value="Assigned" selected>Đã nhận đơn</option>
                                <option value="Processing">Đang xử lý</option>
                              @else
                                <option value="Assigned" disabled>Đã nhận đơn</option>
  ...
  <div class="card">
    <div class="card-header">
      <h4>Quản lý đơn hàng cày thuê - chưa nhận</h4>
    </div>
    <div class="card-body">
      <div class="table-responsive theme-scrollbar">
        <table class="display table table-bordered table-stripped text-nowrap text-center datatablez">
  ...
  <script>
    const claimOrder = async (id) => {
      ...
      try {
        const {
          data: result
        } = await axios.post('{{ route('staff.orders.boostings.claim') }}', {
          id
        });
